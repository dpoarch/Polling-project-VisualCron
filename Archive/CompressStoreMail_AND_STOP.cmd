...
:MAIN
  if exist CompressStoreMail.warn del CompressStoreMail.warn
  @if errorlevel 1 @call :MESS Fatal error deleting CompressStoreMail.warn & goto ERR

  copy CompressStoreMail_run_piece1.txt CompressStoreMail_run.cmd
  @if errorlevel 1 @call :MESS Fatal error copying CompressStoreMail_run_piece1.txt to CompressStoreMail_run.cmd & goto ERR

::  for /F %%E in (\\sghpnas\pos\Poll\StoreMailT\Sitelist.txt) do @call :BUILDCMD %%E || set ERROR_FLAG=TRUE
::  for /F %%E in (\\sghpnas\pos\Poll\Bin\SitelistT.txt) do @call :BUILDCMD %%E || set ERROR_FLAG=TRUE
  for %%E in (\\sghpnas\pos\Poll\StoreMailT\Temp\Mail*.) do @call :BUILDCMD %%~nE || set ERROR_FLAG=TRUE
...
::  for /F %%E in (\\sghpnas\pos\Poll\Bin\SitelistT2.txt) do @call :BUILDCMD2 %%E || set ERROR_FLAG=TRUE
::  @if errorlevel 1 set ERROR_FLAG=TRUE
::  @if ERROR_FLAG==TRUE @call :MESS Fatal error building second piece of CompressStoreMail_run.cmd & goto ERR

  copy CompressStoreMail_run.cmd /A + CompressStoreMail_run_piece2.txt /A CompressStoreMail_run.cmd /A
  ...
  call CompressStoreMail_run.cmd
  @if errorlevel 1 @call :MESS Fatal error compressing store mail & goto ERR

  @if exist CompressStoreMail.warn @call :MESS Error deleting ARS mail for Trovato store & set WARNING_FLAG=TRUE

  @goto CHECKWARN

:BUILDCMD
  @set STORE_NUM=%1
  @echo @call :COMPRESS %STORE_NUM% >> CompressStoreMail_run.cmd
  @echo @if ERRORLEVEL 1 goto ERR >> CompressStoreMail_run.cmd
  @goto :EOF

:BUILDCMD2
  @set STORE_NUM=%1
  @echo @call :DELARS %STORE_NUM% >> CompressStoreMail_run.cmd
  @echo @if ERRORLEVEL 1 goto ERR >> CompressStoreMail_run.cmd
  @goto :EOF
...
::  \\sghpnas\pos\Poll\Bin\CompressPriceCheckFiles
