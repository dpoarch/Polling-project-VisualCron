::------------------------------------------------------------------------------
:: ProcessRecoveredCartTlogs.cmd
::
:: Created by: Jeramie Shake
:: Date: 8/23/2004
::
:: Description:
::     This program will process recovered cart store tlogs.
::
:: Usage: 
::     ProcessRecoveredCartTlogs
::
:: Dependent programs:
::
...
::------------------------------------------------------------------------------
  ...
  @cls
  @echo *********************************
  @echo * ProcessRecoveredCartTlogs.cmd *
  @echo *                               *
  @echo * Created by: Jeramie Shake     *
  @echo *********************************

  @echo.
  @echo Process Recovered cart store tlogs.
  @echo.
::  @pause
...
:MAKEBATCH
  ...
  copy ProcessRecoveredCartTlogs_run_piece1.txt ProcessRecoveredCartTlogs_run.cmd
  @if errorlevel 1 @call :MESS Fatal error copying ProcessRecoveredCartTlogs_run_piece1.txt to ProcessRecoveredCartTlogs_run.cmd & set ERROR_FLAG=TRUE
  @if %ERROR_FLAG%==TRUE goto :EOF

  :: Build and copy in dynamic piece of subprogram.
  for %%E in (\\sghpnas\pos\Data\RecoverT\Cartstores\Tlog*.*) do @call :BUILDCMD %%~nE || set ERROR_FLAG=TRUE
  @if errorlevel 1 set ERROR_FLAG=TRUE
  @if %ERROR_FLAG%==TRUE @call :MESS Fatal error building ProcessRecoveredCartTlogs_run.cmd & set ERROR_FLAG=TRUE
  @if %ERROR_FLAG%==TRUE goto :EOF

  :: Copy in tail piece of subprogram.
  copy ProcessRecoveredCartTlogs_run.cmd /A + ProcessRecoveredCartTlogs_run_piece2.txt /A ProcessRecoveredCartTlogs_run.cmd /A
  @if errorlevel 1 @call :MESS Fatal error copying ProcessRecoveredCartTlogs_run.cmd and ProcessRecoveredCartTlogs_run_piece2.txt to ProcessRecoveredCartTlogs_run.cmd & set ERROR_FLAG=TRUE
  @if %ERROR_FLAG%==TRUE goto :EOF
...
:BUILDCMD
  :: Build and copy in dynamic piece of subprogram.
  @set TLOG_STORE=%1
  @echo @call :FIXTLOGT %TLOG_STORE% >> \\sghpnas\pos\Poll\Bin\ProcessRecoveredCartTlogs_run.cmd
  @echo @if ERRORLEVEL 1 goto ERR >> \\sghpnas\pos\Poll\Bin\ProcessRecoveredCartTlogs_run.cmd
  @goto :EOF
...
:FIXTLOGT
  ...
  call \\sghpnas\pos\Poll\Bin\ProcessRecoveredCartTlogs_run.cmd
...
:ERREND
  ...
  @echo.
  @blat %LOG_FILE% -subject "ERROR: ProcessRecoveredCartTlogs" -to AppDevPOSPollingAlerts@spencergifts.com
