...
:WRITEFLAG
  :: Create ProcessTlogsFinal.flg
  @call :MESS Begin writing ProcessTlogsFinal.flg
  @if exist %PROG_FOLDER%\ProcessTlogsFinal.flg @echo Flag file has been copied already!
  @if not exist %PROG_FOLDER%\ProcessTlogsFinal.flg copy %PROG_FOLDER%\Go.txt %PROG_FOLDER%\ProcessTlogsFinal.flg
  @if errorlevel 1 @call :MESS Fatal error copying %PROG_FOLDER%\Go.txt to %PROG_FOLDER%\ProcessTlogsFinal.flg & set ERROR_FLAG=TRUE
  @if %ERROR_FLAG%==TRUE goto :EOF
  @call :MESS End writing ProcessTlogsFinal.flg
  @goto :EOF
...
:MAKEBATCH
  ...
  copy %PROG_FOLDER%\ProcessTlogsFinal_run_piece1.txt %PROG_FOLDER%\ProcessTlogsFinal_run.cmd
  @if errorlevel 1 @call :MESS Fatal error copying ProcessTlogsFinal_run_piece1.txt to ProcessTlogsFinal_run.cmd & set ERROR_FLAG=TRUE
  @if %ERROR_FLAG%==TRUE goto :EOF
  ...
  @if %ERROR_FLAG%==TRUE @call :MESS Fatal error building ProcessTlogsFinal_run.cmd & set ERROR_FLAG=TRUE
  @if %ERROR_FLAG%==TRUE goto :EOF

  :: Copy in tail piece of subprogram.
  copy %PROG_FOLDER%\ProcessTlogsFinal_run.cmd /A + %PROG_FOLDER%\ProcessTlogsFinal_run_piece2.txt /A %PROG_FOLDER%\ProcessTlogsFinal_run.cmd /A
  @if errorlevel 1 @call :MESS Fatal error copying ProcessTlogsFinal_run.cmd and ProcessTlogsFinal_run_piece2.txt to ProcessTlogsFinal_run.cmd & set ERROR_FLAG=TRUE
  @if %ERROR_FLAG%==TRUE goto :EOF
...
:BUILDCMD
  :: Build and copy in dynamic piece of subprogram.
  @set TLOG_STORE=%1
  @echo @call :DECOMPRESS %TLOG_STORE% >> %PROG_FOLDER%\ProcessTlogsFinal_run.cmd
  @echo @if ERRORLEVEL 1 goto ERR >> %PROG_FOLDER%\ProcessTlogsFinal_run.cmd
  @goto :EOF
...
:DECOMPRESS
  ...
  call %PROG_FOLDER%\ProcessTlogsFinal_run.cmd
...
:: **********************************************************************
  ...
  :: Dave's Translate - Force to run one last time
  c:
  cd\
  cd pollingapps\translate
  C:\PollingApps\translate\Translate.exe SpencerGifts.Translate.Plugin.TLog.Nightly bypassrunningcheck
  @if errorlevel 1 @call :MESS Fatal error in OLD translate & set ERROR_FLAG=TRUE

  C:\PollingApps\translate\Translate.exe SpencerGifts.Translate.Plugin.TLog.Nightly.Consignment bypassrunningcheck
  @if errorlevel 1 @call :MESS Fatal error in OLD translate (consignment) & set ERROR_FLAG=TRUE


  :: OBSOLETE
  ::    :: Dave's NEW Translate - Force to run one last time
  ::    c:
  ::    cd\
  ::    cd pollingapps\Translate_SA5
  ::    C:\PollingApps\Translate_SA5\Translate.exe SpencerGifts.Translate.Plugin.TLog.Nightly 
  ::    @if errorlevel 1 @call :MESS Fatal error in NEW translate & set ERROR_FLAG=TRUE
  ::    
  ::    C:\PollingApps\Translate_SA5\Translate.exe SpencerGifts.Translate.Plugin.TLog.Nightly.Consignment
  ::    @if errorlevel 1 @call :MESS Fatal error in NEW translate (consignment) & set ERROR_FLAG=TRUE


  @if %ERROR_FLAG%==TRUE goto :EOF

  :: Wait 120 seconds
  @call :MESS Wait 120 seconds for the translate to complete...
  %PROG_FOLDER%\Sleep 120
...
:NOMAIL01
  :: Move tlogs retrieved but not processed because store did not receive MAIL01 file.
  @call :MESS Begin move tlogs not processed

  @if exist %POLLING%\Poll\Tlog\Tlog*.* %PROG_FOLDER%\MoveFiles.exe %POLLING%\Poll\Tlog\*.* %POLLING%\Poll\Tlog\NoMail01
  @if errorlevel 1 @call :MESS Error moving files from %POLLING%\Poll\Tlog to %POLLING%\Poll\Tlog\NoMail01 & set WARNING_FLAG=TRUE
  @if exist %POLLING%\Poll\TlogT\Zip\Tlog*.* %PROG_FOLDER%\MoveFiles.exe %POLLING%\Poll\TlogT\Zip\*.* %POLLING%\Poll\TlogT\NoMail01\Zip
  @if errorlevel 1 @call :MESS Error moving files from %POLLING%\Poll\TlogT\Zip to %POLLING%\Poll\TlogT\NoMail01\Zip & set WARNING_FLAG=TRUE

  @call :MESS End move tlogs not processed
  @goto :EOF

:DELETEFLAG
  :: Delete ProcessTlogsFinal.flg
  @call :MESS Begin deleting ProcessTlogsFinal.flg
  @if not exist %PROG_FOLDER%\ProcessTlogsFinal.flg @echo Flag file has been deleted already!
  @if exist %PROG_FOLDER%\ProcessTlogsFinal.flg del %PROG_FOLDER%\ProcessTlogsFinal.flg
  @if errorlevel 1 @call :MESS Fatal error deleting %PROG_FOLDER%\ProcessTlogsFinal.flg & set ERROR_FLAG=TRUE
  @if %ERROR_FLAG%==TRUE goto :EOF
  @call :MESS End deleting ProcessTlogsFinal.flg
  @goto :EOF
...
::  @pause
  ...
  %PROG_FOLDER%\ProcessRecoveredTlogsFinal
...
:ERREND
  ...
  @echo.
  @blat %LOG_FILE% -subject "ERROR: ProcessTlogsFinal" -to AppDevPOSPollingAlerts@spencergifts.com
  ::@pause
