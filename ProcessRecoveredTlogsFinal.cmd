::------------------------------------------------------------------------------
:: ProcessRecoveredTlogsFinal.cmd
::
...
::------------------------------------------------------------------------------
  ...
  @Title Process Recovered Tlogs Final
  @Color 0E
  ...
  @cls
  @echo **********************************
  @echo * ProcessRecoveredTlogsFinal.cmd *
  @echo *                                *
...
:WRITEFLAG
  :: Create ProcessRecoveredTlogsFinal.flg
  @call :MESS Begin writing ProcessRecoveredTlogsFinal.flg
  @if exist %PROG_FOLDER%\ProcessRecoveredTlogsFinal.flg @echo Flag file has been copied already!
  @if not exist %PROG_FOLDER%\ProcessRecoveredTlogsFinal.flg copy %PROG_FOLDER%\Go.txt %PROG_FOLDER%\ProcessRecoveredTlogsFinal.flg
  @if errorlevel 1 @call :MESS Fatal error copying %PROG_FOLDER%\Go.txt to %PROG_FOLDER%\ProcessRecoveredTlogsFinal.flg & set ERROR_FLAG=TRUE
  @if %ERROR_FLAG%==TRUE goto :EOF
  @call :MESS End writing ProcessRecoveredTlogsFinal.flg
  @goto :EOF
...
:MAKEBATCH
  ...
  copy %PROG_FOLDER%\ProcessRecoveredTlogsFinal_run_piece1.txt %PROG_FOLDER%\ProcessRecoveredTlogsFinal_run.cmd
  @if errorlevel 1 @call :MESS Fatal error copying ProcessRecoveredTlogsFinal_run_piece1.txt to ProcessRecoveredTlogsFinal_run.cmd & set ERROR_FLAG=TRUE
  @if %ERROR_FLAG%==TRUE goto :EOF
  ...
  @if %ERROR_FLAG%==TRUE @call :MESS Fatal error building ProcessRecoveredTlogsFinal_run.cmd & set ERROR_FLAG=TRUE
  @if %ERROR_FLAG%==TRUE goto :EOF

  :: Copy in tail piece of subprogram.
  copy %PROG_FOLDER%\ProcessRecoveredTlogsFinal_run.cmd /A + %PROG_FOLDER%\ProcessRecoveredTlogsFinal_run_piece2.txt /A %PROG_FOLDER%\ProcessRecoveredTlogsFinal_run.cmd /A
  @if errorlevel 1 @call :MESS Fatal error copying ProcessRecoveredTlogsFinal_run.cmd and ProcessRecoveredTlogsFinal_run_piece2.txt to ProcessRecoveredTlogsFinal_run.cmd & set ERROR_FLAG=TRUE
  @if %ERROR_FLAG%==TRUE goto :EOF
...
:BUILDCMD
  :: Build and copy in dynamic piece of subprogram.
  @set TLOG_STORE=%1
  @echo @call :DECOMPRESS %TLOG_STORE% >> %PROG_FOLDER%\ProcessRecoveredTlogsFinal_run.cmd
  @echo @if ERRORLEVEL 1 goto ERR >> %PROG_FOLDER%\ProcessRecoveredTlogsFinal_run.cmd
  @goto :EOF
...
:DECOMPRESS
  ...
  call %PROG_FOLDER%\ProcessRecoveredTlogsFinal_run.cmd
...
:: **********************************************************************
  ...
  C:\PollingApps\Translate\Translate.exe SpencerGifts.Translate.Plugin.TLog.Daily bypassrunningcheck
  @if errorlevel 1 @call :MESS Fatal error in NEW translate & set ERROR_FLAG=TRUE

  C:\PollingApps\Translate\Translate.exe SpencerGifts.Translate.Plugin.TLog.Daily.Consignment bypassrunningcheck
  ...
  ::    C:\PollingApps\Translate_SA5\Translate.exe SpencerGifts.Translate.Plugin.TLog.Daily 
  ::    @if errorlevel 1 @call :MESS Fatal error in NEW translate & set ERROR_FLAG=TRUE
  ::    
  ::    C:\PollingApps\Translate_SA5\Translate.exe SpencerGifts.Translate.Plugin.TLog.Daily.Consignment
  ::    @if errorlevel 1 @call :MESS Fatal error in NEW translate (consignment) & set ERROR_FLAG=TRUE


  @if %ERROR_FLAG%==TRUE goto :EOF

  :: Wait 120 seconds  Added By Dave Stein 9/7/2007
...
:TESTMOVE
  ...
  if exist %POLLING%\Data\RecoverT\Good\Converted\Tlog*.* %PROG_FOLDER%\MoveFiles.exe %POLLING%\Data\RecoverT\Good\Converted\Tlog*.* %POLLING%\Data\RecoverT\Good\Backup
  @if errorlevel 1 @call :MESS Fatal error translating Trovato tlogs & set ERROR_FLAG=TRUE
  @if %ERROR_FLAG%==TRUE goto :EOF
...
:DELETEFLAG
  :: Delete ProcessRecoveredTlogsFinal.flg
  @call :MESS Begin deleting ProcessRecoveredTlogsFinal.flg
  @if not exist %PROG_FOLDER%\ProcessRecoveredTlogsFinal.flg @echo Flag file has been deleted already!
  @if exist %PROG_FOLDER%\ProcessRecoveredTlogsFinal.flg del %PROG_FOLDER%\ProcessRecoveredTlogsFinal.flg
  @if errorlevel 1 @call :MESS Fatal error deleting %PROG_FOLDER%\ProcessRecoveredTlogsFinal.flg & set ERROR_FLAG=TRUE
  @if %ERROR_FLAG%==TRUE goto :EOF
  @call :MESS End deleting ProcessRecoveredTlogsFinal.flg
  @goto :EOF
...
::  @pause
  ...
  :: Continue with the next Batch File
    ::Step skipped as of 7/22/2013
    ::%PROG_FOLDER%\SetPollingDoneFlag
...
:ERREND
  ...
  @echo.
  @blat %LOG_FILE% -subject "ERROR: ProcessRecoveredTlogsFinal" -to AppDevPOSPollingAlerts@spencergifts.com
  ::@pause
